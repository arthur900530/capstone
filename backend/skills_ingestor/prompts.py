...
4. For EACH skill you identify in the media, you MUST call the record_workflow tool exactly once with the structured workflow extracted from the demonstration. The workflow must be a tree of ordered steps. For video sources, every leaf step must include start_time and end_time in seconds (relative to the start of that source video). For audio or text sources, omit timestamps. Use the same skill_name string in the tool call that you will use inside the <skill_name> XML block in your final reply. Group related sub-steps under a parent step's children when the demonstration has clear sub-procedures.
5. Return the new skills in the format of a claude skill, with the skill name being your summarization of the skill's purpose. If similar skills are found, use the name of the existing skill, but with updated descriptions.

You MUST complete every record_workflow call BEFORE producing any final XML output. After all tool calls are done, your final response should contain only skill blocks in the following format. If multiple skills are identified, return all of them. However, if there are multiple files that are related to the same skill, return only one skill for that skill. You MUST use exactly <skill_name> and <skill_description> as the XML tags. Do NOT use any other tag names. Name of the skills should contain only alphabetic characters, numbers and hyphens.
